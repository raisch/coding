...
_.mixin({
  isPositiveNumber: val => _.isNumber(val) && val > 0,
  canDivideByThree: val => val % 3 == 0,
  canDivideByFive: val => val % 5 == 0,
  isNonEmptyArray: val => _.isArray(val) && !_.isEmpty(val)
})
...
const naive = function (max = MAX, cb = noop) {
  ...
  if (cb) {
    setImmediate(cb, null, 'naive', result)
  }
}
...
const usingLodash = function (max = MAX, cb = noop) {
  ...
  setImmediate(cb, null, 'usingLodash', result)
}

/**
 * Parameterized implementation of FizzBuzz.
 * <br/><br/>
 * Passing range, mutators, and callback.
 *
...
const parameterized = function (range = [], mutators = [], cb = noop) {
  ...
  setImmediate(cb, null, 'parameterized', result)
}
...
const functional = function (range = [], mutators = [], cb = noop) {
  ...
  setImmediate(cb, null, 'functional', result)
}
...
const events = function (range = [], mutators = [], cb = noop) {
  ...
  const handlers = {
    ...
    nextNumber: opts => { // process next number in range
    ...
    nextMutator: opts => { // process next mutator in mutators using num
    ...
    complete: opts => { // print results
      // console.log(`complete: ${JSON.stringify(opts, null, 2)}`)
      setImmediate(cb, null, 'events', opts.results)
    }

  }

  E.on('start', () => setImmediate(handlers.start))
  E.on('nextNumber', opts => setImmediate(handlers.nextNumber, opts, opts))
  E.on('nextMutator', opts => setImmediate(handlers.nextMutator, opts, opts))
  E.on('complete', opts => setImmediate(handlers.complete, opts, opts))
...
const promised = function (range = [], mutators = [], cb = noop) {
  ...
  Promise.all(promises)
    .then(res => setImmediate(cb, null, 'promised', res))
    .catch(e => {
 ...
 * @typedef FizzBuzz
 * @type {Array}
 * @property {String} 0 - result if test resolves to true
 * @property {Function} 1 - test
 */
